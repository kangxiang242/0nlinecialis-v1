...
@section('content')
    ...
    <div class="goods">
            ...
            <nav class="sku">
                ...
                <ul class="spec-main">
                    @foreach($categorys as $key=>$item)
                        <li class="spec-item {{ $item->uri==request()->path()?"activate":"" }}"><a href="{{ URL::to($item->uri) }}">犀利士 {{ $item->name }}</a ></li>
                    @endforeach
                </ul>
            </nav>
        </header>
        ...
        <section class="card">
            <h2 class="hero-title">犀利士組合線上訂購</h2>
            <ul class="products">
                @foreach($category->products as $goods)
                    <li>
                        <div class="quantity">
                            <h3 class="goods-name"><span class="cate">犀利士壯陽藥 {{ $category->name }}</span><span class="break">{{ $goods->name }}</span></h3>

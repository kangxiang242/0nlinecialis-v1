...
<head>
    ...
    @section('style')
        <link rel="stylesheet" type="text/css" href="{{ assetv('gb-static/css/style.css') }}"/>
        <link rel="stylesheet" type="text/css" href="{{ assetv('gb-static/css/common.css') }}"/>
        <link rel="stylesheet" type="text/css" href="{{ assetv('gb-static/less/global.css') }}"/>
        <link rel="stylesheet" href="{{ assetv('gb-static/font_3122894_ix34x1wtlao/iconfont.css') }}">
        <link rel="stylesheet" href="{{ assetv('gb-static/swiper4/swiper.min.css') }}">
        <link rel="stylesheet" href="{{ assetv('gb-static/less/section.css') }}">
    @show
    <script src="{{ assetv('gb-static/js/jquery.min.js') }}"></script>
    <script>
...
<header class="main-header">
    ...
    <nav class="nav-sec">
        <button class="menu-btn nav-heading close-btn" onclick="closeMobileMenu()">
            <span class="text">關閉選單</span>
            <i class="iconfont">&#xeca0;</i>
        </button>
        <ul class="base">
            <li class="link-parent" tabindex="0">
                <h2 class="base-link">犀利士 Cialis<sup>®</sup>&nbsp;線上訂購<i class="iconfont">&#xeca2;</i></h2>
                <ul class="mega-menu">
                    @foreach($categorys as $item)
                    <li class="link-item"><a href="{{ $item->uri?url($item->uri):"javascript:;" }}">犀利士 Cialis<sup>®</sup>&nbsp;{{ $item->name }}</a></li>
                    @endforeach
                </ul>
            </li>
            @foreach($navigations as $nav)
                <li class="link-parent" tabindex="0">
                    <h2 class="base-link">{{ $nav->name_gb }}<i class="iconfont">&#xeca2;</i></h2>
                    @if($nav->sub && count($nav->sub))
                        <ul class="mega-menu">
                            @foreach($nav->sub as $sub)
                            <li class="link-item"><a href="{{ $sub->link?url($sub->link):"javascript:;" }}">{{ $sub->name_gb }}</a></li>
                            @endforeach
                        </ul>
                    @endif
                </li>
    ...
    <button class="menu-btn" onclick="showMobileMenu()">
...
<main>
    @section('page-header')
        ...
        @endphp
        <aside class="ad-carousel swiper-container">
            <div class="swiper-wrapper">
                @foreach($categorys as $item)
                <article class="swiper-slide" itemscope itemtype="https://schema.org/Product">
                    <img class="ad-pic" itemprop="image" src="{{ storage_url($item->img) }}" alt="{{ $item->describe }} 犀利士Cialis® {{ $item->name }} 產品圖片展示">
                    <div class="ad-text">
                        <h2 class="p1" itemprop="name"><span>犀利士 Cialis<sup>®</sup> {{ $item->name }}</span>{{ $item->describe_gb }}</h2>
                        <p class="p2" itemprop="description">{{ $item->describe2_gb }}</p>
                    </div>
                    <a href="{{ URL::to($item->uri) }}" itemprop="url" class="ad-btn">
                        <span>立即訂購</span>
                        <i class="iconfont">&#xe684;</i>
                    </a>
                </article>
            ...
            <button class="close-btn" onclick="closeAside()" aria-label="關閉">
                <i class="iconfont">&#xeca0;</i>
            </button>
        </aside>
        <header class="page-header {{ $class }}">
            <div class="page-header-content">
                <h1>{{ isset($page)?$page->title_gb:"" }}</h1>
                <p class="sub">@yield('billboard-desc_gb')</p>
            </div>
            @if($banner)
...
<footer class="main-footer">
    <div class="logo-sec">
        ...
        <a href="https://www.facebook.com/sharer/sharer.php?u=https://0nlinecialis.com" target="_blank" rel="noopener noreferrer nofollow">
        ...
        <a href="https://www.linkedin.com/sharing/share-offsite/?url=https://0nlinecialis.com" target="_blank" rel="noopener noreferrer nofollow">
        ...
        <a href="https://social-plugins.line.me/lineit/share?url=https://0nlinecialis.com" target="_blank" rel="noopener noreferrer nofollow">
    ...
    <nav class="main-links">
        <h2 class="links-title">禮來犀利士 Cialis<sup>®</sup>&nbsp;線上訂購：</h2>
        <ul class="links-list">
            @foreach($categorys as $item)
                <li class="links-item"><a href="{{ $item->uri?url($item->uri):"javascript:;" }}">犀利士 Cialis<sup>®</sup>&nbsp;{{ $item->name }}</a></li>
            @endforeach
        </ul>
    </nav>
...
@section('script')
<script src="{{ assetv('gb-static/js/less.min.js') }}"></script>
<script src="{{ assetv('gb-static/swiper4/swiper.min.js') }}"></script>
<script src="{{ assetv('gb-static/js/jquery.cookie.js') }}"></script>
<script src="{{ assetv('gb-static/js/xie.js') }}"></script>

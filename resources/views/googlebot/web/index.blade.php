...
@section('content')
    <section>
        <header class="sec-header">
            ...
            <a class="more" href="{{ URL::to('about') }}" title="點擊了解更多 關於犀利士壯陽藥">了解更多 關於犀利士壯陽藥</a>
        </header>
        <img class="float-image" src="{{ storage_url($setting->get('home_pill_img')) }}" alt="壯陽藥犀利士藥丸展示圖片" loading="lazy">
    ...
    <section class="channel">
        ...
        <section class="dosage-list">
            <h3 class="subtitle">線上訂購犀利士劑量選擇</h3>
            @foreach($categorys as $item)
                <article class="dosage" itemscope itemtype="https://schema.org/Product">
                    <img class="ad-pic" itemprop="image" loading="lazy" src="{{ storage_url($item->img) }}" alt="犀利士Cialis® {{ $item->name }} 線上訂購圖片展示 {{ $item->describe }}">
                    <div class="ad-text">
                        <h4 class="p1" itemprop="name"><span class="highlight">{{ $item->describe_gb }}</span><span>犀利士 Cialis<sup>®</sup> {{ $item->name }}</span></h4>
        ...
        @foreach($faqs as $faq)
            ...
            <details class="faq-item">
                <summary class="faq-question">
                    <span class="question-text">{{ $faq->questions_gb }}</span>
                    <i class="iconfont faq-icon">&#xeca2;</i>
                </summary>
                <p class="faq-answer">{{ $faq->answers_gb }}</p>
            </details>
            @endif
        @endforeach
    ...
    <section class="news">
        <header class="sec-header">
            ...
            <a class="more" href="{{ URL::to('blog') }}" title="點擊了解更多 犀利士壯陽藥新聞">了解更多 犀利士壯陽藥新聞</a>
        </header>

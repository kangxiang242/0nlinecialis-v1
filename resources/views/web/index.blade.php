...
@section('content')
    <section class="container first-container">
        <header class="sec-title">
            ...
            <a class="more" href="{{ URL::to('about') }}" title="點擊了解更多 關於禮來犀利士壯陽藥">了解更多 關於禮來犀利士壯陽藥</a>
        </header>
        <img class="float-image" src="{{ ($pill = $setting->get('home_pill_img')->value()) ? storage_url($pill) : '' }}" alt="犀利士壯陽藥藥丸展示" loading="lazy">
        <div class="image wow animate__animated animate__fadeInUp"><img class="scale-img" src="{{ ($about = $setting->get('home_about_img')->value()) ? storage_url($about) : '' }}" alt="{{ $setting->get('home_lilly_about_title') }}"></div>
    </section>
    ...
    <section class="channel container">
        ...
        <section class="dosage-list container">
            <h3 class="subtitle">線上訂購犀利士壯陽藥劑量選擇</h3>
            @foreach($categorys as $item)
                <article class="dosage" itemscope itemtype="https://schema.org/Product">
                    <img class="ad-pic" itemprop="image" loading="lazy" src="{{ storage_url($item->img) }}" alt="犀利士Cialis® {{ $item->name }} 線上訂購產品圖片展示">
                    <div class="ad-text">
                        <h4 class="p1" itemprop="name"><span class="highlight">{{ $item->describe }}</span><span>犀利士 Cialis<sup>®</sup> {{ $item->name }}</span></h4>
                        <p class="p2" itemprop="description">{{ $item->describe2 }}</p>
                    </div>
                    <a href="{{ URL::to($item->uri) }}" itemprop="url" class="ad-btn" title="查看 犀利士 {{ $item->name }} 線上訂購優惠組合">
                        <span>查看 犀利士 {{ $item->name }} 優惠組合</span>
                        <i class="iconfont">&#xe684;</i>
                    </a>
                </article>
            @endforeach
    ...
    <section class="faq-section container">
        <header class="sec-title">
            <h2 class="title">犀利士壯陽藥顧客常見疑問</h2>
        </header>
        @foreach($faqs as $faq)
            ...
            @endif
        @endforeach
        <a class="more text-underline" href="{{ URL::to('faq') }}" title="了解更多 犀利士壯陽藥顧客常見疑問">了解更多 犀利士壯陽藥顧客常見疑問</a>
    </section>
    <section class="news container">
        <header class="sec-title">
            <h2 class="title">{{ $setting->get('home_news_about_title') }}</h2>
            <p class="text">{{ $setting->get('home_news_about_desc') }}</p>
            <a class="more" href="{{ URL::to('blog') }}" title="點擊了解更多 犀利士壯陽藥使用心得">了解更多 犀利士壯陽藥使用心得</a>
        </header>

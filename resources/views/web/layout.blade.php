    ...
    <nav class="nav-sec" aria-label="主選單">
        ...
        <div class="base">
            <div class="link-parent" tabindex="0">
                <p class="base-link">犀利士 Cialis<sup>®</sup>&nbsp;線上訂購<i class="iconfont">&#xeca2;</i></p>
                <div class="mega-menu">
                    @foreach($categorys as $item)
                    <div class="link-item"><a href="{{ $item->uri?url($item->uri):"javascript:;" }}">犀利士 Cialis<sup>®</sup>&nbsp;{{ $item->name }}</a></div>
                    @endforeach
                </div>
            </div>
...
@section('page-header')
    ...
    @endphp
    <aside class="ad-carousel swiper-container" aria-label="犀利士線上訂購推薦">
        <div class="swiper-wrapper">
            @foreach($categorys as $item)
            <article class="swiper-slide" itemscope itemtype="https://schema.org/Product">
                <img class="ad-pic" itemprop="image" src="{{ storage_url($item->img) }}" alt="犀利士Cialis® {{ $item->name }} 線上訂購產品圖片展示">
                <div class="ad-text">
                    <h2 class="p1" itemprop="name"><span>犀利士 Cialis<sup>®</sup> {{ $item->name }}</span>{{ $item->describe }}</h2>
                    <p class="p2" itemprop="description">{{ $item->describe2 }}</p>
                </div>
                <a href="{{ URL::to($item->uri) }}" itemprop="url" class="ad-btn" title="立即訂購犀利士 {{ $item->name }}">
                    <span>立即訂購</span>
                    <i class="iconfont">&#xe684;</i>
                </a>
            </article>
...
<footer>
    ...
    <nav class="main-links" aria-label="犀利士線上訂購連結">
        <p class="links-title">禮來犀利士 Cialis<sup>®</sup>&nbsp;線上訂購：</p>
        <div class="links-list">
            @foreach($categorys as $item)
                <p class="links-item"><a href="{{ $item->uri?url($item->uri):"javascript:;" }}">犀利士 Cialis<sup>®</sup>&nbsp;{{ $item->name }}</a></p>
            @endforeach
        </div>
    </nav>
...
@section('script')
<script src="{{ assetv('static/js/less.min.js') }}"></script>

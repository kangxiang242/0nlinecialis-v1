...
@section('breadcrumb')
    <nav aria-label="Breadcrumb">
        <ul class="breadcrumb">
            ...
            <li class="active">線上訂購犀利士壯陽藥 {{ $category->name }}</li>
        </ul>
    </nav>
@stop
...
@section('content')
    ...
    <section class="goods">
        <header class="info-wrap">
            ...
            <section class="indication">
                <h2 class="title">犀利士壯陽藥功效與適應症說明：</h2>
            ...
            <nav class="sku" aria-label="犀利士劑量選擇">
                <h2 class="title">犀利士劑量選擇：</h2>
                <div class="spec-main">
                    @foreach($categorys as $key=>$item)
                        <p class="spec-item {{ $item->uri==request()->path()?"activate":"" }}"><a href="{{ URL::to($item->uri) }}">犀利士 {{ $item->name }}</a ></p>
                    @endforeach
                </div>
            </nav>
        </header>
    ...
    <section class="prescribe">
        <h2 class="hero-title">犀利士壯陽藥處方訊息</h2>
        @php
            $prescribes = [];
            try {
    ...
    <section class="faq-section">
        <h2 class="hero-title">犀利士壯陽藥常見問題</h2>
        @foreach($faqs as $faq)
        ...
        <div class="news-container">
            @foreach($news->where('article_cate_id',4)->take(3) as $item)
                <article class="news-item">
                    <a href="{{ URL::to($item->cate->uri.'/'.$item->id) }}" title="閱讀 {{ $item->title }}全部內容">

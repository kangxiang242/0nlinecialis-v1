...
class NewsController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $news = $this->newsRepository->model()->where('article_cate_id',$cate->id)->orderBy('sort','desc')->paginate(6);

        $recommend = $this->newsRepository->all()->where('article_cate_id',$cate->id)->shuffle()->take(6);



        return view('web::news.index')->with('news',$news)->with('cate',$cate)->with('recommend',$recommend);
    }
